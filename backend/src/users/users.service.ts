...
export class UsersService {
  ...
  public static readonly LDAP_ERROR_RESPONSE = `
      ...
      remaining name: 'OU=Users,O=DemonSlayers'
    `;
  ...
  async createUser(user: UserDto, isBasicUser = true): Promise<User> {
    ...
    u.role = (user.role as any) || 'people';
  ...
  async getAllUsers(): Promise<User[]> {
    this.log.debug(`Called getAllUsers`);
    return this.usersRepository.findAll();
  }

  async getUsersByRole(role: UserRole): Promise<User[]> {
    this.log.debug(`Called getUsersByRole ${role}`);
    return this.usersRepository.find({ role });
  }

  async updateUserRole(email: string, role: UserRole): Promise<User> {
    this.log.debug(`Called updateUserRole ${email} to ${role}`);
    const user = await this.findByEmail(email);
    user.role = role;
    await this.em.persistAndFlush(user);
    return user;
